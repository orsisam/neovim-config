...
function ToggleNumberVisualSelection()
	...
	-- Check if any laine for numbering
...
function ToggleCheckboxVisualSelection()
	...
	if has_checkboxes then
		...
		for i = 1, #lines do
			if lines[i]:match("^%s*%-%s") then
				-- Convert existing bullets to checkbox
				lines[i] = lines[i]:gsub("^(%s*%-)%s*", "%1 [ ] ")
...
-- ** Header Colors **
-- highlight for markdown files to render highlights properly
...
vim.cmd(
	string.format([[highlight @markup.heading.4.mardown cterm=bold gui=bold guifg=%s guibg=%s]], color_fg, color4_bg)
)
vim.cmd(
